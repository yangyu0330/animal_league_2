    ...
    <div className="mobile-canvas min-h-screen bg-background">
      <main className="px-4 py-6">
        <header className="mb-6 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">과제 압박을 느낄 때 학과를 탭하고 전국 랭킹을 확인하세요.</p>
        </header>
        ...
        <section className="mb-6">
          ...
          <div className="space-y-2">
            {previewItems.map((department, index) => (
              <article key={department.departmentId} className="flex h-[72px] items-center gap-3 rounded-2xl border border-border bg-card px-3">
                <p className="number-display w-8 text-center text-lg font-bold text-primary">{index + 1}</p>
                <div className="min-w-0 flex-1">
                  <p className="truncate text-sm font-semibold text-foreground">{department.departmentName}</p>
                  <p className="truncate text-xs text-muted-foreground">{department.schoolName}</p>
                </div>
                <div className="text-right">
                  <p className="number-display text-sm font-bold text-foreground">{department.totalClicks.toLocaleString()}</p>
      ...
      <div className="sticky bottom-0 border-t border-border bg-background p-4 safe-bottom">
        ...
        >
          {isSigningIn ? '로그인 연결 중...' : 'Google로 시작하기'}
        </Button>
      </div>
    </div>
  )
